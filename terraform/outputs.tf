...
output "set_webhook_command" {
  sensitive = true
  value = <<-EOT
    curl -X POST "https://api.telegram.org/bot${var.bot_token}/setWebhook" -H "Content-Type: application/json" -d '{"url": "${aws_lambda_function_url.webhook.function_url}", "drop_pending_updates": true}'
  EOT
}